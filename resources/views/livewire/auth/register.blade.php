...
<div class="relative flex h-dvh w-full items-center justify-center overflow-hidden bg-slate-100 px-4 sm:px-6 lg:px-8">
    ...
    <div class="relative z-10 w-full max-w-md">
        <div class="rounded-2xl border-2 border-slate-300 bg-white p-4 shadow-2xl shadow-slate-400/40 ring-2 ring-slate-200/70 backdrop-blur-xl sm:p-5 [@media(max-height:820px)]:p-4">
            ...
            <div class="mb-4 text-center [@media(max-height:820px)]:mb-3">
                <h1 class="text-2xl font-bold text-slate-900 [@media(max-height:820px)]:text-xl">Create Account</h1>
                <p class="mt-1 text-sm text-slate-600 [@media(max-height:820px)]:mt-0.5">Register to access your TruthGuard workspace</p>
            </div>

            @if (session('auth_error'))
                <div class="mb-4 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700 [@media(max-height:820px)]:mb-3">
                    {{ session('auth_error') }}
                </div>
            @endif

            <form method="POST" action="{{ route('register') }}" class="space-y-2.5 [@media(max-height:820px)]:space-y-2">
                @csrf
                <div>
                    <label for="name" class="mb-1 block text-sm font-semibold text-slate-700 [@media(max-height:820px)]:mb-0.5">Name</label>
                    <div class="relative">
                        ...
                        <input
                            ...
                            value="{{ old('name') }}"
                            autocomplete="name"
                            class="w-full rounded-xl border border-slate-300 bg-white py-2.5 pl-10 pr-3 text-sm text-slate-900 placeholder:text-slate-400 focus:border-cyan-500 focus:outline-none focus:ring-4 focus:ring-cyan-100 [@media(max-height:820px)]:py-2"
                ...
                <div>
                    <label for="email" class="mb-1 block text-sm font-semibold text-slate-700 [@media(max-height:820px)]:mb-0.5">Email</label>
                    <div class="relative">
                        <span class="pointer-events-none absolute inset-y-0 left-3 flex items-center text-slate-400">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" class="h-4 w-4">
                ...
                <div>
                    <label for="password" class="mb-1 block text-sm font-semibold text-slate-700 [@media(max-height:820px)]:mb-0.5">Password</label>
                    <div
                    ...
                    >
                        ...
                        <input
                            ...
                            class="w-full rounded-xl border border-slate-300 bg-white py-2.5 pl-10 pr-12 text-sm text-slate-900 placeholder:text-slate-400 focus:border-cyan-500 focus:outline-none focus:ring-4 focus:ring-cyan-100 [@media(max-height:820px)]:py-2"
                ...
                <div>
                    <label for="password_confirmation" class="mb-1 block text-sm font-semibold text-slate-700 [@media(max-height:820px)]:mb-0.5">Confirm Password</label>
                    <div
                        x-data="{ showPassword: false, hasValue: false }"
                        x-init="$nextTick(() => { hasValue = $refs.confirmPasswordInput.value.length > 0; if (hasValue) { $refs.confirmPasswordInput.dispatchEvent(new Event('input', { bubbles: true })); } })"
                        class="relative"
                    >
                        <span class="pointer-events-none absolute inset-y-0 left-3 flex items-center text-slate-400">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" class="h-4 w-4">
                        ...
                        <input
                            ...
                            x-bind:type="showPassword ? 'text' : 'password'"
                            x-on:input="hasValue = $refs.confirmPasswordInput.value.length > 0"
                            autocomplete="new-password"
                            class="w-full rounded-xl border border-slate-300 bg-white py-2.5 pl-10 pr-12 text-sm text-slate-900 placeholder:text-slate-400 focus:border-cyan-500 focus:outline-none focus:ring-4 focus:ring-cyan-100 [@media(max-height:820px)]:py-2"
            ...
            <p class="mt-4 text-center text-sm text-slate-600 [@media(max-height:820px)]:mt-3">
                Already have an account?
                <a href="{{ route('login') }}" class="font-semibold text-cyan-700 transition hover:text-cyan-600">Login</a>
            </p>
        </div>
    </div>
</div>
